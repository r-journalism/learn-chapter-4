...
ggplot(data=ages) +
   ...
   geom_abline(intercept=1, col="light gray") 
...
We start with with the **data frame**. We initialized the dataviz function `ggplot()` by passing the **ages** data frame to it.
...
There's gotta be some mnemonic device to remember this, right? Someone let me know.
...
Each function in **ggplot2** like `geom_abline()` allows for additional variables that can be passed to it. In this example, we specificy the intercept should be at 1 and the color of the bar should be "light gray". We accept the default width of the line by not passing it the variable to the `geom_abline()` at all.
...
Data works best in `ggplot()` in a tidy, long format.
...
Notice that we are nesting `aes()` into `ggplot()` this time and not using the `method=` because it's not necesary.
...
So `geom_jitter()` is a way to show distribution. The location is grouped by the y, which in this instance is the actor. 

```{r jitter}
ggplot(ages, aes(x=actress_age, y=actor)) +
  geom_jitter()
```

Let's throw in another variable into `aes()` from the data frame: Genre.
...
It makes more sense to apply scalar transformations to while plotting rather than altering the dataset itself.
...
Remember how that big scatterplot above had all the different colors on one viz and it was all clustered together?
...
We needed a second data set without the *actor* variable to plot in a background layer and set the color manually to "grey". This was the first `geom_point()` layer referenced locally.
